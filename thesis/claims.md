...
| With A40's ball requirement on, the detector-first rule reaches only 0.311 ACTIVE_PLAY recall across the seven held-out clip venues, scored one frame at a time inside one camera's boundary. It was 0.996 before the requirement (2026-09-19); the requirement, not the detector, is what changed. | `0.311` | `results/rule_frame_eval.csv` | `TODO.md` | ✔ |
...
| Without the ball requirement the rule's worst held-out venue is 0.970, where the probe's is 0.667. The probe's mean hides a fold it fails badly; the counting rule's does not — until A40's ball requirement is applied, which opens a worse fold of its own (see rule-ball-requirement-worst-venue). | `0.9702` | `results/rule_frame_eval.csv` | `TODO.md` | ✔ |
