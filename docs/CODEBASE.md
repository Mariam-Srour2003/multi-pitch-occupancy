...
main                     planning documents only
  └── setup/project-scaffold
        └── feat/dataset-manifest
              └── ... 39 more branches, each stacked on the last ...
                    └── feat/class-balancing-check      <- the tip: everything is here
```

**The tip branch (`feat/class-balancing-check`) contains the whole project** — 64 commits,
46 source modules, 17 experiment scripts, 27 test files.

```bash
git checkout feat/class-balancing-check
```
...
```bash
git checkout main && git merge feat/class-balancing-check
```
