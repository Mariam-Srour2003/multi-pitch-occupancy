...
main                     planning documents only
  └── setup/project-scaffold
        └── feat/dataset-manifest
              └── ... 40 more branches, each stacked on the last ...
                    └── fix/search-false-play-control      <- the tip: everything is here
```

**The tip branch (`fix/search-false-play-control`) contains the whole project** — 68 commits,
46 source modules, 18 experiment scripts, 28 test files.

```bash
git checkout fix/search-false-play-control
```
...
```bash
git checkout main && git merge fix/search-false-play-control
```
...
| 40 | `fix/search-false-play-control` | 1 | Repair the search's false-play control; it was scoring probes on their own training data |
<!-- branch-table:end -->
