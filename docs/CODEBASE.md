...
main                     PLAN.md · SUPER_PLAN.md · TODO.md · README.md · 2 .docx · .gitignore · requirements.txt
  └── setup/project-scaffold
        └── feat/dataset-manifest
              └── ... 32 more branches, each stacked on the last ...
                    └── docs/branch-reference-refresh      <- the tip: everything is here
```

**The tip branch (`docs/branch-reference-refresh`) contains the whole project** — 56 commits,
45 source modules, 15 experiment scripts, 24 test files.
...
```bash
git checkout main && git merge docs/branch-reference-refresh
```
...
| 33 | `docs/branch-reference-refresh` | 1 | **Tip.** This block, generated from git so it stops drifting |
...
```bash
git checkout docs/branch-reference-refresh  # the code lives here, not on main
