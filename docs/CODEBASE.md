...
<!-- branch-report:start -->
`main` carries the whole project — 47 source modules,
22 experiment scripts, 29 test files. Clone it and everything is
...
The work was built on a chain of 47 stacked feature branches, each on top of the last, and
...
main  <- everything is here
  └── history: setup/project-scaffold → feat/dataset-manifest → ... → 47 branches, each stacked on the last
```
