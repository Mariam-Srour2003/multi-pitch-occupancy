...
<!-- branch-report:start -->
`main` carries the whole project — 61 commits, 46 source
modules, 16 experiment scripts, 26 test files. Clone it and everything is
...
The work was built on a chain of 37 stacked feature branches, each on top of the last, and
...
main  <- everything is here
  └── history: setup/project-scaffold → feat/dataset-manifest → ... → 37 branches, each stacked on the last
```
