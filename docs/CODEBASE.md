...
31 experiment scripts, 39 test files. Clone it and everything is
...
The work was built on a chain of 73 stacked feature branches, each on top of the last, and
...
main  <- everything is here
  └── history: setup/project-scaffold → feat/dataset-manifest → ... → 73 branches, each stacked on the last
```
...
uv run pytest -q -m "not slow"      # 622 tests
```
