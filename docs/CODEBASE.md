...
19 experiment scripts, 29 test files. Clone it and everything is
...
The work was built on a chain of 42 stacked feature branches, each on top of the last, and
...
main  <- everything is here
  └── history: setup/project-scaffold → feat/dataset-manifest → ... → 42 branches, each stacked on the last
```
