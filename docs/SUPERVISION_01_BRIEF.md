...
### What is actually done to each image — preprocessing and augmentation

A likely question, and there are **two separate stages**. The honest headline, which you
should give before you are asked: *most of the published numbers use almost none of stage 1,
and stage 2 was never in the main benchmark at all.*

**There are pictures of all of this**, on the Augmentation tab of the thesis site
(`/#augmentation`): every preprocessing switch at the model's real 224×224 input, every
augmentation effect on its own, and every preset. If a switch is questioned in the room, show
the tile rather than describing it. All three sheets are regenerated by
`experiments/augmentation_grid.py` and the faces in them are pixelated before anything is drawn.

**Stage 1 — preprocessing (`vision/preprocess.py`).** One entry point for both the
experiments and the live pipeline, because the pilot's most expensive bug came from those two
paths differing. The order is fixed: **geometry → photometric → resize → post-resize.**

| stage | switch | what it does |
|---|---|---|
| geometry | `undistort` | approximate barrel-distortion correction — the footage is heavily fisheye and the lens differs per venue, so geometry is itself a venue cue |
| | `roi` | mask to a pitch polygon. **Never used — no polygon has been drawn yet**, so it is deliberately excluded from the search rather than evaluated as a no-op |
| | `top_crop` | discard a fraction from the top only — sky, stands and adjacent pitches sit above the horizon |
| | `centre_crop` | keep the central fraction of the frame |
| photometric | `per_image_standardise` | z-score the frame, removing the global brightness offset that separates a daylight morning from a floodlit night |
| | `clahe` (`off`/`on`/`auto`) | local contrast equalisation on the L channel in LAB, so colour is not shifted. `auto` gates on RMS contrast — and that gate fires on **78% of frames**, so it is a weak switch, not a vacuous one |
| | `gamma` | brighten or darken; night frames crush shadows |
| | `saturation` | scale colour; `0.0` is full grayscale |
| | `denoise` | bilateral filter — smooths night-sensor noise without losing edges |
| resize | `letterbox` | aspect-preserving resize to 224 with grey padding, vs. a squashing resize. Worth ~0.03 recall on this fisheye footage |
| post-resize | `sharpen` | unsharp mask, countering fisheye softness at the frame edge |
| | `blur_sigma` | Gaussian blur. **A diagnostic, not a setting** — it removes the detail that people are made of, so a score that survives it was never about people |

**No rotation, warp or perspective change at any point.** The cameras are bolted to a post.

> **If they ask which of these the results use:** the main benchmark uses **none of them**.
> `build_cache` hands *raw* frames to the HuggingFace processor, whose own resize is the only
> thing making them model-sized — and for ConvNeXtV2 and DINOv2 that means resize to 256 and
> centre-crop back to 224, **discarding 23.4% of the frame**. Only four experiments call
> `preprocess` themselves: the input ablation, the preprocessing search, the resolution
> search and the geometry probe. This is a known and documented gap, not an oversight — and
> the one result that challenged a model on this input path was tested and **did not survive**
> (section 5, item 3).

**What the input ablation found, with its correction attached** (`results/input_ablation.csv`):

- **Removing information has a floor, and only one removal is above it.** Grayscale is the
  real finding: it takes the false-play rate from **0.230 to 0.021** at **0.979** empty
  accuracy — better on *both* axes.
- A centre crop looked like the bigger win (+0.038 macro-F1 against grayscale's +0.022) and
  **was not**: 0.998 play recall at **empty accuracy 0.000**, on folds containing no empty
  pitch to catch it. Corrected on 2026-09-11 when the ablation gained its false-play control.
- The two combined fall *below* the untouched baseline and call **98.8%** of empty pitches a
  match. Blur at σ=4 calls **100%** of them a match.

**Stage 2 — augmentation (`vision/augment.py`), training only.** Nine effects, each sampled
per image and gated at probability `p`; all default to off so a run must opt in.

| | |
|---|---|
| `brightness` | ± fraction of full scale — floodlights vary between venues and across a night |
| `contrast` | ± fraction — haze, glare and lens dirt all flatten contrast |
| `saturation` | ± fraction — turf hue is a venue cue that does not transfer |
| `hue` | ± degrees, small only — a large shift turns grass a colour no pitch has |
| `gamma` | ± exponent offset — covers both ends of night footage |
| `noise` | Gaussian sigma in 8-bit levels — night sensors are noisy |
| `fog` | synthetic haze, blended toward bright grey and **heavier toward the top of the frame**, because in a fixed pitch camera distance increases up the frame |
| `rain` | synthetic oriented motion-blurred streaks, one angle per image, plus the contrast drop wet air causes |
| `flip` | horizontal mirror |

Five presets: `none`, `colour` (brightness/contrast/saturation/hue), `weather` (fog/rain/noise),
`light` (brightness/gamma/noise), `full` (everything).

Three things to say about it without being asked:

- **Augmentation is the complement to preprocessing, and that is why it exists.** Preprocessing
  *removes* information permanently and has a floor. Augmentation varies it and **keeps every
  pixel at inference**, so it targets the same shortcut — turf hue, lighting — without crossing
  that floor.
- **Horizontal flip is the one geometric exception, deliberately.** It produces a mirror the
  camera never sees, which is exactly why it is useful: it cannot change whether people are
  playing, but it breaks memorisation of *this particular pitch's* layout — the failure the
  whole cross-venue evaluation exists to catch.
- **The rain is drawn, not filmed. Nothing in the dataset is wet.** So a model trained on it
  has *not* been shown to work in real rain, and I do not claim it has.

**Never applied at inference.** Augmenting a live frame would make the same image classify
differently on two runs, which an audit trail cannot survive.

**And it is not in the main benchmark, for a stated reason.** Everything downstream trains a
linear probe on *cached* embeddings, one vector per frame. Augmentation happens before the
backbone, so every augmented view needs its own forward pass — the cache stops being a cache,
at roughly 8 minutes per model per epoch-equivalent against seconds for a cached probe fit. So
it ships tested (16 property tests) and **unclaimed**. The one place it was measured is
`augmentation_transfer.py`, on the cross-camera boundary — and that is the result in section 5
that turned out to be the best of five seeds and was retracted.
...
**"What preprocessing and augmentation did you apply?"**
Full list in section 2. The short answer, and give the qualifier with it: preprocessing is a
searchable pipeline — undistort, ROI, crops, standardise, CLAHE, gamma, saturation, denoise,
letterbox, sharpen, blur — but **the main benchmark applies none of it** and lets the
HuggingFace processor resize. Augmentation is photometric jitter, synthetic fog and rain,
noise and a horizontal flip; **no rotations or warps, because the cameras are bolted down**,
and it is train-time only. It is not in the benchmark, and the one result that used it was
retracted.
